...
public class MarkdownEngine extends MarkupEngine {
    ...
    public void processBody(final ParserContext context) {
				...
				for (DataKey<?> optionKey : optionKeys) {
					...
					case "java.lang.Integer":
						try {
							...
							logger.warn("Value '{}' is not a valid Ineger Value for option {}", optionValue, optionKey);
						}
						break;
        ...
        logger.info("Makdown Options : " + allOptions);
	...
	/**
	 * Classe owning the attribute. Can be NULL but not recommended (all Owner with that atribut will be set with the value)
	 * @param onwer class owning the property to change (must extends com.vladsch.flexmark.IRender or com.vladsch.flexmark.IParser). Can be NULL.
	...
	private List<DataKey<?>> getDataValueSetter(String onwer, String fieldName){
		...
		if(returnVal.isEmpty()) {
			logger.warn("No option(s) '{}.{}' found in extension Holders : {}" , onwer, fieldName, flexMarkOptionsHolderClasses);
		}
	...
	private DataKey<?> extractFieldValue(Class<?> classHolder, String fieldName) {
		...
		try {
			...
			logger.warn("Cannot instanciate option '{}.{}', this option will be ignored, cause '{}'", classHolder, fieldName, e);
		}
		
		return returnVal;
	}
	
	private <T> void processEnumOption(MutableDataSet allOptions, Class myEnum, DataKey<T> optionKey, String optionValue) {
		try {
			allOptions.set((DataKey<T>)optionKey, (@NotNull T) Enum.valueOf(myEnum, optionValue) );
		}catch (IllegalArgumentException e) {
			logger.warn("Cannot manage option of type : {} value {} is not a valid value. Allowed values {}", myEnum.getName(), optionValue, myEnum.getEnumConstants());
		}
	}
	...
	private Map<String, String> convertOptionValueToMap(String optionValue) {
	    ...
	    if (returnVal.isEmpty() && hasError) {
	    	...
	    	logger.warn("Map contain only invalid values : map will use default value");
	    	returnVal = null;
	    }
